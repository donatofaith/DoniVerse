...
export default function DiscoverPage() {
  ...
  useEffect(() => {
    ...
    async function loadDiscover() {
      const [eventsResult, communitiesResult] = await Promise.all([
        supabase
          .from("events")
          .select("id,title,description,event_type,starts_at,ends_at,venue_name,organizer_name,official_url,image_url,is_featured")
          .order("starts_at", { ascending: true }),
        supabase
          .from("communities")
          .select("id,name,category,description,location_name,contact_url,official_url,image_url,is_verified")
  ...
  const eventGroups = useMemo(() => {
    ...
    for (const event of events) {
      ...
      if (start <= now && end >= now) happeningNow.push(event);
      else if (start > now && start <= weekEnd) thisWeek.push(event);
      else if (start > weekEnd) upcoming.push(event);
    }
      ...
      <div className="relative mx-auto w-full max-w-[1120px] px-4 pb-8 pt-5 sm:px-6 md:px-8 lg:px-10">
        <header className="rounded-[28px] border border-white/60 bg-white/38 p-5 shadow-[0_20px_60px_rgba(16,46,28,0.08)] backdrop-blur-3xl dark:border-white/[0.08] dark:bg-white/[0.04] sm:p-6">
          <div className="flex items-center justify-between gap-4">
            <div>
              <div className="inline-flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.14em] text-[#397151] dark:text-[#8ce6ad]"><Compass size={15} /> Discover DoniVerse</div>
            ...
            <div className="hidden h-12 w-12 items-center justify-center rounded-[17px] border border-white/60 bg-white/46 text-[#34744c] dark:border-white/[0.08] dark:bg-white/[0.06] dark:text-[#9bedb7] sm:flex"><Sparkles size={21} /></div>
          </div>
          ...
          <section className="mt-5">
            <div className="rounded-[28px] border border-white/55 bg-white/32 p-5 shadow-[0_22px_65px_rgba(16,46,28,0.09)] backdrop-blur-3xl dark:border-white/[0.08] dark:bg-white/[0.04] sm:p-6">
              <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <div className="flex h-11 w-11 items-center justify-center rounded-[15px] border border-white/60 bg-white/48 text-[#34744c] dark:border-white/[0.08] dark:bg-white/[0.06] dark:text-[#9bedb7]"><Users size={22} /></div>
                  <h2 className="mt-5 text-[27px] font-black tracking-[-0.045em]">Find your people.</h2>
                  <p className="mt-2 max-w-[620px] text-sm leading-6 text-black/50 dark:text-white/48">Student associations, fellowships, academic groups, clubs, creative communities and more.</p>
                </div>
                <a href="/discover/submit-community" className="inline-flex min-h-12 shrink-0 items-center justify-center gap-2 rounded-[17px] bg-[#174d31] px-5 text-sm font-black text-white shadow-[0_16px_44px_rgba(23,77,49,0.20)] dark:bg-[#9bedb7] dark:text-[#0b2717]"><Plus size={17} /> Add your community</a>
              </div>
            </div>
...
function EventCard({ event }: { event: EventRecord }) {
  ...
  const date = start.toLocaleDateString("en-NG", { weekday: "short", day: "numeric", month: "short" });
  const startTime = start.toLocaleTimeString("en-NG", { hour: "numeric", minute: "2-digit" });
  const endTime = end ? end.toLocaleTimeString("en-NG", { hour: "numeric", minute: "2-digit" }) : null;
...
function LoadingCard({ label }: { label: string }) {
  return <div className="mt-6 flex min-h-[120px] items-center justify-center gap-3 rounded-[24px] border border-white/55 bg-white/28 text-sm font-bold text-black/45 backdrop-blur-3xl dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-white/40"><LoaderCircle size={18} className="animate-spin" /> {label}</div>;
}
...
function EmptyCard({ icon, title }: { icon: React.ReactNode; title: string }) {
  return <div className="mt-5 rounded-[24px] border border-dashed border-white/60 bg-white/26 p-8 text-center backdrop-blur-2xl dark:border-white/[0.08] dark:bg-white/[0.025]"><div className="mx-auto w-fit text-black/25 dark:text-white/25">{icon}</div><p className="mt-3 text-sm font-bold">{title}</p></div>;
}
