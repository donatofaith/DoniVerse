  ...
  useEffect(() => {
    ...
    async function loadEvent() {
      ...
      const { data, error: loadError } = await supabase
        .from("events")
        .select(
          "id,title,description,event_type,starts_at,ends_at,venue_name,venue_place_id,organizer_name,official_url,image_url,inquiry_name,inquiry_phone",
        )
        .eq("id", params.id)
        .eq("status", "approved")
        .maybeSingle();
      ...
      if (loadError || !data) {
        setError("This event is unavailable or has not been approved yet.");
        setEvent(null);
      } else {
    ...
    return (
      <main className="flex min-h-[100dvh] items-center justify-center bg-transparent px-5 text-[#102017] dark:text-white">
        <div className="rounded-[25px] border border-white/60 bg-white/55 px-7 py-6 text-center shadow-xl backdrop-blur-3xl dark:border-white/10 dark:bg-[#0b1410]/72">
          ...
          <p className="mt-3 text-sm font-bold">Loading event...</p>
        </div>
      </main>
    );
  }
    ...
    return (
      <main className="flex min-h-[100dvh] items-center justify-center bg-transparent px-5 pb-24 text-[#102017] dark:text-white">
        <section className="w-full max-w-lg rounded-[30px] border border-white/60 bg-white/50 p-7 text-center shadow-xl backdrop-blur-3xl dark:border-white/10 dark:bg-[#0b1410]/72">
          <CalendarDays size={29} className="mx-auto text-[#397151] dark:text-[#9bedb7]" />
          <h1 className="mt-4 text-2xl font-black">Event unavailable</h1>
    ...
    <main className="relative min-h-[100dvh] overflow-x-hidden bg-[#e9efe9] pb-28 text-[#102017] dark:bg-[#050b07] dark:text-white">
      ...
      <div className="relative mx-auto w-full max-w-[960px] px-4 pb-10 pt-[max(16px,env(safe-area-inset-top))] sm:px-6 md:px-8">
        ...
        <article className="mt-6 overflow-hidden rounded-[34px] border border-white/60 bg-white/42 shadow-[0_28px_90px_rgba(16,46,28,0.13)] backdrop-blur-3xl dark:border-white/10 dark:bg-[#0b1410]/66">
          {event.image_url ? (
            <div className="relative max-h-[620px] overflow-hidden bg-black/[0.05] dark:bg-black/25">
              <div
              ...
              <img
                ...
                className="relative mx-auto max-h-[620px] w-full object-contain"
              />
            </div>
          ) : (
            <div className="flex min-h-[230px] items-center justify-center bg-[radial-gradient(circle_at_30%_20%,rgba(155,237,183,0.22),transparent_45%),linear-gradient(145deg,rgba(255,255,255,0.3),rgba(255,255,255,0.10))] dark:bg-[radial-gradient(circle_at_30%_20%,rgba(155,237,183,0.12),transparent_45%),linear-gradient(145deg,rgba(255,255,255,0.05),rgba(255,255,255,0.01))]">
              <CalendarDays size={48} className="text-[#397151]/55 dark:text-[#9bedb7]/65" />
            </div>
          )}
          ...
          <div className="p-5 sm:p-7 md:p-8">
            ...
            <div className="mt-7 grid gap-3 sm:grid-cols-2">
              <InfoCard icon={<CalendarDays size={18} />} label="Date" value={date} />
              <InfoCard
                icon={<Clock3 size={18} />}
                label="Time"
              ...
              />
              {event.venue_name && (
                <InfoCard icon={<MapPin size={18} />} label="Venue" value={event.venue_name} />
              )}
              <InfoCard icon={<UserRound size={18} />} label="Organiser" value={event.organizer_name} />
            </div>

            <div className="mt-7 grid gap-3 sm:grid-cols-2">
              <a
                href={`/api/calendar/event/${event.id}`}
              ...
              {event.official_url && (
                ...
                >
                  More information <ExternalLink size={16} />
                </a>
              )}
            ...
            {(event.inquiry_name || event.inquiry_phone) && (
              <div className="mt-7 rounded-[22px] border border-white/60 bg-white/42 p-4 backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.04]">
                <p className="text-[10px] font-black uppercase tracking-[0.1em] text-black/35 dark:text-white/30">Public inquiry contact</p>
                <p className="mt-1 text-sm font-black">
                  {[event.inquiry_name, event.inquiry_phone].filter(Boolean).join(" · ")}
                </p>
              </div>
            )}
          </div>
...
function InfoCard({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="rounded-[20px] border border-white/60 bg-white/42 p-4 backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.04]">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          <p className="text-[10px] font-black uppercase tracking-[0.1em] text-black/35 dark:text-white/30">{label}</p>
