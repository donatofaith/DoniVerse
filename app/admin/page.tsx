    ...
    return (
      <main className="flex min-h-[100dvh] items-center justify-center bg-transparent px-5 text-[#102017] dark:text-white">
        <div className="w-full max-w-md rounded-[30px] border border-white/60 bg-white/55 p-6 text-center shadow-[0_24px_70px_rgba(16,46,28,0.12)] backdrop-blur-3xl dark:border-white/10 dark:bg-[#0b1410]/72">
          ...
          <p className="mt-2 text-sm leading-6 text-black/50 dark:text-white/45">This area is only available to FUTAGO administrators.</p>
          <button type="button" onClick={() => router.replace("/")} className="mt-5 min-h-12 rounded-[16px] border border-white/65 bg-white/55 px-5 text-sm font-extrabold text-[#214f34] dark:border-white/10 dark:bg-white/[0.06] dark:text-white">
            Back to FUTAGO
          </button>
        </div>
      </main>
    ...
    <main className="relative min-h-[100dvh] bg-transparent pb-12 text-[#102017] dark:text-white">
      <div className="mx-auto w-full max-w-[1080px] px-4 pb-10 pt-[max(16px,env(safe-area-inset-top))] sm:px-6 md:px-8">
        <header className="flex items-center justify-between gap-4">
          <button type="button" onClick={() => router.push("/")} className="flex h-11 w-11 items-center justify-center rounded-full border border-white/65 bg-white/50 shadow-sm backdrop-blur-2xl dark:border-white/10 dark:bg-white/[0.06]" aria-label="Back to Home">
            <ArrowLeft size={18} />
          </button>
        ...
        <section className="mt-6 rounded-[32px] border border-white/60 bg-white/45 p-5 shadow-[0_26px_80px_rgba(16,46,28,0.10)] backdrop-blur-3xl dark:border-white/10 dark:bg-[#0b1410]/62 sm:p-7 md:p-8">
          <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <h1 className="mt-2 text-[34px] font-black leading-none tracking-[-0.055em] sm:text-[46px]">Manage FUTAGO.</h1>
              <p className="mt-3 max-w-xl text-sm leading-6 text-black/50 dark:text-white/45">Manage campus content and trusted administrators from one place.</p>
            </div>
        ...
        <button type="button" onClick={() => router.push("/admin/events")} className="mt-5 flex w-full items-center justify-between rounded-[26px] border border-white/60 bg-white/42 p-5 text-left shadow-[0_20px_60px_rgba(16,46,28,0.08)] backdrop-blur-3xl transition active:scale-[0.995] dark:border-white/10 dark:bg-[#0b1410]/58 sm:p-6">
          <div className="flex items-center gap-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-[16px] border border-white/60 bg-white/52 text-[#397151] dark:border-white/10 dark:bg-white/[0.06] dark:text-[#9bedb7]">
              <CalendarDays size={21} />
            </div>
            <div>
              <p className="text-lg font-black tracking-[-0.03em]">Manage events</p>
              <p className="mt-1 text-xs text-black/42 dark:text-white/38">Approve, reject, create and edit campus events.</p>
            </div>
          </div>
          ...
          <section className="mt-5 rounded-[28px] border border-white/60 bg-white/42 p-5 shadow-[0_20px_60px_rgba(16,46,28,0.08)] backdrop-blur-3xl dark:border-white/10 dark:bg-[#0b1410]/58 sm:p-6">
            ...
            <form onSubmit={addAdmin} className="mt-5 grid gap-3 md:grid-cols-[1fr_190px_auto]">
              <div className="flex min-h-[56px] items-center gap-3 rounded-[18px] border border-white/65 bg-white/48 px-4 backdrop-blur-xl focus-within:border-[#69a87f]/50 dark:border-white/10 dark:bg-[#101914]/70">
                ...
                <input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="student@example.com" className="min-w-0 flex-1 bg-transparent text-base font-semibold outline-none placeholder:text-black/28 dark:text-white dark:placeholder:text-white/25" />
              </div>

              <select value={newRole} onChange={(event) => setNewRole(event.target.value as AdminRole)} className="min-h-[56px] rounded-[18px] border border-white/65 bg-white/80 px-4 text-base font-bold text-[#102017] outline-none dark:border-white/10 dark:bg-[#101914] dark:text-white">
                <option value="admin">Admin</option>
                <option value="super_admin">Super Admin</option>
              </select>

              <button type="submit" disabled={saving} className="flex min-h-[56px] items-center justify-center gap-2 rounded-[18px] border border-[#77b68d]/35 bg-[#dff3e5]/90 px-5 text-sm font-black text-[#17462d] shadow-sm transition active:scale-[0.99] disabled:opacity-60 dark:border-[#8ce6ad]/20 dark:bg-[#8ce6ad]/15 dark:text-[#a9efc1]">
        ...
        <section className="mt-5 rounded-[28px] border border-white/60 bg-white/42 p-4 shadow-[0_20px_60px_rgba(16,46,28,0.08)] backdrop-blur-3xl dark:border-white/10 dark:bg-[#0b1410]/58 sm:p-6">
          ...
          <div className="space-y-3">
            {admins.map((admin) => {
              ...
              return (
                <div key={admin.user_id} className="flex flex-col gap-3 rounded-[20px] border border-white/55 bg-white/38 p-4 backdrop-blur-xl dark:border-white/8 dark:bg-white/[0.035] sm:flex-row sm:items-center sm:justify-between">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="truncate text-sm font-black">{admin.email ?? "Unknown email"}</p>
                      {isYou && <span className="rounded-full bg-[#dff3e5] px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.08em] text-[#286140] dark:bg-[#8ce6ad]/15 dark:text-[#a9efc1]">You</span>}
                    </div>
                    <p className="mt-1 text-xs font-semibold capitalize text-black/40 dark:text-white/35">{admin.role === "super_admin" ? "Super admin" : "Admin"}</p>
                  </div>

                  {role === "super_admin" && !isYou && (
                    <button type="button" onClick={() => void removeAdmin(admin)} disabled={removingId === admin.user_id} className="flex min-h-11 items-center justify-center gap-2 rounded-[15px] border border-red-400/15 bg-red-400/[0.07] px-4 text-xs font-black text-red-700 transition disabled:opacity-50 dark:text-red-300">
                      {removingId === admin.user_id ? <Loader2 size={15} className="animate-spin" /> : <Trash2 size={15} />}
                      Remove
                    </button>
                  )}
                </div>
