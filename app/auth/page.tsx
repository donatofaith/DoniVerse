...
export default function AuthPage() {
  ...
  const handlePasswordReset =
    async (
      ...
      if (!cleanedEmail) {
        setError(
          "Enter the email connected to your FUTAGO account."
        );
        return;
      }
      ...
      >
        ...
        <div>
          <div className="mb-2 flex items-center justify-between gap-3">
            <label
              htmlFor="password"
              className="text-sm font-semibold text-black/65 dark:text-white/65"
            >
              Password
            </label>
            ...
            {!loading && (
              <span className="mt-0.5 block text-xs text-white/55 dark:text-[#082013]/55">
                ...
                "signup"
                  ...
                  : "Continue to your FUTAGO profile"}
              </span>
            )}
          </div>
...
function ErrorBox({
  ...
  return (
    <div
      role="alert"
      className="rounded-[16px] border border-red-500/15 bg-red-500/[0.07] px-4 py-3 text-sm font-medium leading-5 text-red-700 dark:text-red-300"
    >
      {text}
    </div>
  );
...
function SuccessBox({
  ...
  return (
    <div
      role="status"
      className="rounded-[16px] border border-emerald-500/15 bg-emerald-500/[0.08] px-4 py-3 text-sm font-medium leading-5 text-emerald-800 dark:text-emerald-200"
    >
      {text}
    </div>
  );
